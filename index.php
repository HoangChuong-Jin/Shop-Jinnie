...
$loaisanpham = $loaimp->layloaisanpham();
$sanpham = $mp->laysanpham();
$sanphamnoibat = $mp->laysanphamnoibat();
...
switch($action){
    ...
    case "xulydangnhap":
        ...
        $admin = $nd->kiemtranguoidunghople($email,$matkhau);
        if($khachhang){
    ...
    case "xemtatcasanpham": 
        $tongmh = $mp->demtongsosanpham(); //tổng số mặt hàng
        ...
        $sanpham = $mp->laysanphamtrongkhoang($batdau,$soluong);
        include("tatcasanpham.php"); 
        break;
    case "xemloaisanpham": 
        if(isset($_REQUEST["maloai"])){
            $maloai = $_REQUEST["maloai"];
            $loai = $loaimp->layloaisanphamtheoid($maloai);
            $tenloai =  $loai["tenloai"];   
            $sanpham = $mp->laysanphamtheoloaisanpham($maloai);
    ...
    case "xemchitiet": 
        if(isset($_GET["id"])){
            ...
            $mpct = $mp->laysanphamtheoid($id);
            // lấy các mỹ phẩm cùng loại
            $loaisanpham_id = $mpct["loaisanpham_id"];
            //lấy tên loại
            $loai = $loaimp->layloaisanphamtheoid($loaisanpham_id);
            $tenloai =  $loai["tenloai"]; 
            //lấy mỹ phẩm cùng loại
            $sanpham = $mp->laysanphamtheoloaisanpham($loaisanpham_id);
            include("detail.php");
        }
        break;
    case "themvaogio":
    ...
    case "capnhatgiohang":
        $mh=$_REQUEST["mp"];
        foreach($mh as $sanpham => $soluong)
        {
            if($soluong>0)
                capnhatsoluong($sanpham, $soluong);
            else
                xoamotmathang($sanpham);
        }
        $giohang=laygiohang();
        include("cart.php");
        break;
    ...
    case "luudonhang":
        ...
        else
        {
            if($kh->kiemtraemail($email)==FALSE)
            {
                ...
                $message="Bạn đã sử dụng email này cho một tài khoản! Vui lòng thay đổi email hoặc đăng nhập với email hiện tại.";
                $giohang=laygiohang();
                include("checkout.php");
                break;
            }
